...
def simg(ra=37.228,dec=0.37,
    ...
    savename=savename,init_download=init_download)

    if show == True:
        jpg.show()


def DownFile(file, scale=0.396, width=512, height=512, show=False):
...
# creating directories
    ...
    for idx, item in enumerate(name):
        ...
        simg(ra=ra[idx], dec=dec[idx], show=show, savename=namfil, scale=scale, width=width, height=height)
...
class SdssJpg(object):
    ...
    def __init__(self, ra, dec,
                 ...
                 savename=None, init_download=True):
        self.ra = ra
        self.dec = dec
        self.scale = scale
        self.width = width
